...
async function applyXcodeChanges(config, project, props) {
    ...
    if (props.disableAutolinking) {
        ...
        console.log(`[@bacons/apple-targets] Auto-linking disabled for "${props.name}" - target will not be embedded or added as dependency`);
    }
    else if (props.parentTarget) {
        // Find the specified parent target by name
        parentTargetForLinking = (0, target_1.findTargetByName)(project, props.parentTarget);
        if (!parentTargetForLinking) {
            const availableTargets = project.rootObject.props.targets
                .filter((t) => xcode_1.PBXNativeTarget.is(t))
                .map((t) => t.props.name ||
                t.props.productName)
                .join(", ");
            throw new Error(`[@bacons/apple-targets] Could not find parent target "${props.parentTarget}" for extension "${props.name}". ` +
                `Available targets: ${availableTargets}`);
        }
        console.log(`[@bacons/apple-targets] Embedding "${props.name}" into parent target: ${parentTargetForLinking.props.name || parentTargetForLinking.props.productName}`);
    }
    else {
    ...
    if (parentTargetForLinking) {
        ...
        const WELL_KNOWN_COPY_EXTENSIONS_NAME = (() => {
            ...
            else if (targetToUpdate.props.productType ===
